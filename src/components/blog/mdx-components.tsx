...
function H2({ children }: { children?: ReactNode }) {
  return (
    <h2 id={slugifyHeading(textOf(children))} className="mt-12 scroll-mt-28 font-serif text-2xl font-medium text-ink sm:text-3xl">
      {children}
    </h2>
  )
}
...
function PriceNote() {
  return (
    <span className="mt-8 flex items-start gap-2 rounded-xl border border-divider bg-white px-4 py-3 text-sm text-muted">
...
function Callout({ children }: { children?: ReactNode }) {
  return (
    <div className="my-6 rounded-2xl border border-accent/20 bg-accent/[0.06] p-6 text-ink/85">
      {children}
    </div>
  )
}
...
function MoneyPageCTA({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="my-6 inline-flex items-center gap-2 rounded-full bg-accent px-6 py-3 font-medium text-white transition-colors hover:bg-accent-dark"
    >
...
export const mdxComponents = {
  ...
  h3: H3,
  a: Anchor,
  p: (p: { children?: ReactNode }) => <p className="mt-6 leading-relaxed text-ink/80" {...p} />,
  ul: (p: { children?: ReactNode }) => <ul className="mt-6 list-disc space-y-2 pl-6 text-ink/80" {...p} />,
  ol: (p: { children?: ReactNode }) => <ol className="mt-6 list-decimal space-y-2 pl-6 text-ink/80" {...p} />,
  ...
  blockquote: (p: { children?: ReactNode }) => (
    <blockquote className="my-6 border-l-4 border-accent/40 pl-6 italic text-ink/75" {...p} />
  ),
  ...
  th: (p: { children?: ReactNode }) => (
    <th className="border-b border-divider bg-background px-4 py-2 font-semibold text-ink" {...p} />
  ),
  td: (p: { children?: ReactNode }) => <td className="border-b border-divider px-4 py-2 text-ink/80" {...p} />,
  // Shortcodes
  Price,
  Money: MoneyTag,
  PriceNote,
